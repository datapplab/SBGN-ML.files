...
all.kegg.tre = readChar(fileName, file.info(fileName)$size)
head(all.kegg.tre)
length(all.kegg.tre)
all.kegg.tre[1]
all.kegg.tre = strsplit(all.kegg.tre,"'")[[1]]
length(all.kegg.tre)
head(all.kegg.tre)
tail(all.kegg.tre)
t.names = all.kegg.tre[seq(2,(length(all.kegg.tre)-1),by=2)]
head(t.names)
tail(t.names)
...
# check orthologs in human, mouse and chimpanzee
ko.to.uniprot = read.table("C:/xiaoxi/work/input/id.mapping/kegg.related/kegg.ortholog.100.download.slice/uniprot.to.ko.tsv",header=T,stringsAsFactors = F,sep="\t")
head(ko.to.uniprot)
human.kos = unique(ko.to.uniprot[ko.to.uniprot[,"species"]=="hsa",][,"ko"])
head(human.kos)
length(human.kos)
human.kos.three.species = unique(ko.to.uniprot[ko.to.uniprot[,"species"] %in% c("ptr","hsa","mmu") & ko.to.uniprot[,"ko"] %in% human.kos,2:3])
head(human.kos.three.species)
table(human.kos.three.species[,"species"])
