...
# pathway.cutoff.anova = ddply(
        # pathway.species.pct_Mapped[1:1000,]
        # ,as.factor(pathway.species.pct_Mapped$pathway)[1:1000]
        pathway.species.pct_Mapped
        ,as.factor(pathway.species.pct_Mapped$pathway)
        ,function(df){
# df = pathway.species.pct_Mapped[pathway.species.pct_Mapped$pathway == "P00002",]
...
out.mean.as.cutoff= out[grepl("number",out[,"stat"]),]
...
```{r}
out.anova.as.cutoff = out[!grepl("number",out[,"stat"]),] 
...
if.max.F = by(
        ...
        ,function(df){
                cutoffs = as.numeric(df[,"cutoff"])
                if.max = cutoffs == max(cutoffs)
